// ignore_for_file: must_be_immutable

import 'package:equatable/equatable.dart';
import 'recommendation_item_model.dart';

/// This class defines the variables used in the [recommendation_screen],
/// and is typically used to hold data that is passed between different parts of the application.
class RecommendationModel extends Equatable {
  RecommendationModel({this.recommendationItemList = const []}) {}

  List<RecommendationItemModel> recommendationItemList;

  RecommendationModel copyWith(
      {List<RecommendationItemModel>? recommendationItemList}) {
    return RecommendationModel(
      recommendationItemList:
          recommendationItemList ?? this.recommendationItemList,
    );
  }

  @override
  List<Object?> get props => [recommendationItemList];
}
